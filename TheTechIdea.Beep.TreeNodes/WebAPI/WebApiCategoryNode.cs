...
{
    [AddinAttribute(Caption = "WebAPI", BranchType = EnumPointType.Category, Name = "WebApiCategoryNode.Beep", misc = "Beep", iconimage = "category.png", menu = "Beep", ObjectType = "Beep")]
    public class WebApiCategoryNode : IBranch 
    {
        public WebApiCategoryNode() { }

        public WebApiCategoryNode(ITree pTreeEditor, IDMEEditor pDMEEditor, IBranch pParentNode, string pBranchText, int pID, EnumPointType pBranchType, string pimagename)
        {
        ...
        public IErrorsInfo CreateChildNodes()
        {
            try
            {
                ...
                foreach (CategoryFolder p in DMEEditor.ConfigEditor.CategoryFolders.Where(x => x.RootName.Equals("WEBAPI", StringComparison.InvariantCultureIgnoreCase) && x.FolderName == BranchText))
                {
        ...
        public IErrorsInfo CreateDBNode(ConnectionProperties i)
        {
            try
            {
                ...
                WebApiNode database = new WebApiNode(i, TreeEditor, DMEEditor, this, i.ConnectionName, TreeEditor.SeqID, EnumPointType.DataPoint, icon);
