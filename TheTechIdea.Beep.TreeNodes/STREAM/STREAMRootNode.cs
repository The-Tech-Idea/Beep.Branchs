...
namespace TheTechIdea.Beep.TreeNodes.STREAM
{
    ...
    {
        public STREAMRootNode()
        {
            IsDataSourceNode = true;
        }

        public STREAMRootNode(ITree pTreeEditor, IDMEEditor pDMEEditor, IBranch pParentNode, string pBranchText, int pID, EnumPointType pBranchType, string pimagename)
        {
        ...
        public string BranchText { get; set; } = "STREAM";
        ...
        public IErrorsInfo CreateChildNodes()
        {
            try
            {
                foreach (ConnectionProperties i in DMEEditor.ConfigEditor.DataConnections.Where(c => c.Category == DatasourceCategory.STREAM && c.IsComposite == false))
                {
                    if (TreeEditor.Treebranchhandler.CheckifBranchExistinCategory(i.ConnectionName, "STREAM") == null)
                    {
