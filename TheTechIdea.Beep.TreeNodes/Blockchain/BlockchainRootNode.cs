...
namespace TheTechIdea.Beep.TreeNodes.Blockchain
{
    ...
    {
        public BlockchainRootNode()
        {
            IsDataSourceNode = true;
        }

        public BlockchainRootNode(ITree pTreeEditor, IDMEEditor pDMEEditor, IBranch pParentNode, string pBranchText, int pID, EnumPointType pBranchType, string pimagename)
        {
        ...
        public string BranchText { get; set; } = "Blockchain";
        ...
        public IErrorsInfo CreateChildNodes()
        {
            try
            {
                foreach (ConnectionProperties i in DMEEditor.ConfigEditor.DataConnections.Where(c => c.Category == DatasourceCategory.Blockchain && c.IsComposite == false))
                {
                    if (TreeEditor.Treebranchhandler.CheckifBranchExistinCategory(i.ConnectionName, "Blockchain") == null)
                    {
