...
namespace TheTechIdea.Beep.TreeNodes.Connector
{
    ...
    {
        public ConnectorRootNode()
        {
            IsDataSourceNode = true;
        }

        public ConnectorRootNode(ITree pTreeEditor, IDMEEditor pDMEEditor, IBranch pParentNode, string pBranchText, int pID, EnumPointType pBranchType, string pimagename)
        {
        ...
        public string BranchText { get; set; } = "Connector";
        ...
        public IErrorsInfo CreateChildNodes()
        {
            try
            {
                foreach (ConnectionProperties i in DMEEditor.ConfigEditor.DataConnections.Where(c => c.Category == DatasourceCategory.Connector && c.IsComposite == false))
                {
                    if (TreeEditor.Treebranchhandler.CheckifBranchExistinCategory(i.ConnectionName, "Connector") == null)
                    {
