...
{
    [AddinAttribute(Caption = "MLModel", BranchType = EnumPointType.Root, Name = "MLModelRootNode.Beep", misc = "Beep", iconimage = "mlmodel.svg", menu = "DataSource", ObjectType = "Beep", Category = DatasourceCategory.MLModel)]
    [AddinVisSchema(BranchType = EnumPointType.Root, BranchClass = "DATASOURCEROOT", RootNodeName = "DataSourcesRootNode")]
    public class MLModelRootNode : IBranch  
    {
        public MLModelRootNode()
        {
            IsDataSourceNode = true;
        }

        public MLModelRootNode(ITree pTreeEditor, IDMEEditor pDMEEditor, IBranch pParentNode, string pBranchText, int pID, EnumPointType pBranchType, string pimagename)
        {
        ...
        public string BranchText { get; set; } = "MLModel";
        ...
        public string IconImageName { get; set; } = "mlmodel.svg";
        ...
        public IErrorsInfo CreateChildNodes()
        {
            try
            {
                foreach (ConnectionProperties i in DMEEditor.ConfigEditor.DataConnections.Where(c => c.Category == DatasourceCategory.MLModel && c.IsComposite == false))
                {
                    if (TreeEditor.Treebranchhandler.CheckifBranchExistinCategory(i.ConnectionName, "MLModel") == null)
                    {
